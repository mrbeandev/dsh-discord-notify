window.__ModuleLoader__.load({ id: 'dsh-discord-notify', factory: (require) => {
  ...
  function validateWebhookDraft(value) {
    if (!value) return
    let url
    try { url = new URL(value) } catch { throw new Error('Discord webhook URL is not a valid URL.') }
    const host = ['discord.com', 'discordapp.com', 'canary.discord.com', 'ptb.discord.com'].includes(url.hostname)
    if (url.protocol !== 'https:' || !host || !/^\/api(?:\/v\d+)?\/webhooks\/[^/]+\/[^/]+\/?$/.test(url.pathname)) throw new Error('Use an HTTPS Discord webhook URL such as https://discord.com/api/webhooks/…')
  }
  ...
  function DiscordNotifyCard({ useScope, scope }) {
    ...
    const [webhookUrl, setWebhookUrl] = useState(''); const [notifyTurnStart, setNotifyTurnStart] = useState(false); const [notifyTurnEnd, setNotifyTurnEnd] = useState(true)
    ...
    const load = (next, nextRevision) => { const value = next && typeof next === 'object' ? next : {}; setBase(value); setRevision(nextRevision); setWebhookUrl(''); setNotifyTurnStart(value.notifyTurnStart === true); setNotifyTurnEnd(value.notifyTurnEnd !== false); setNotifyToolCalls(value.notifyToolCalls === true); setToolNames(Array.isArray(value.toolNames) ? value.toolNames : []); setNotifyBashMatches(value.notifyBashMatches === true); setBashRegex(typeof value.bashRegex === 'string' ? value.bashRegex : ''); setIncludeToolArguments(value.includeToolArguments === true); setUsername(typeof value.username === 'string' ? value.username : 'DeepSeek Harness'); setTemplates(Object.fromEntries(Object.entries(TEMPLATE_SPECS).map(([key, spec]) => [key, typeof value.templates?.[key] === 'string' ? value.templates[key] : spec.fallback]))) }
    ...
    const verifyWrite = (beforeRevision, ops) => { const accepted = scope.getSnapshot(); const visible = ops.filter((op) => op.path[0] !== 'webhookUrl'); const visibleRejected = visible.length > 0 && (accepted.revision === beforeRevision || visible.some((op) => JSON.stringify(accepted.value?.[op.path[0]]) !== JSON.stringify(op.value))); if (visibleRejected) throw new Error('The Host rejected the visible settings. Check the regex, selected tools, and template variables.'); return accepted }

    const save = async () => { setSaving(true); setFailed(''); setSuccess(''); try { const before = scope.getSnapshot(); if (before.revision !== revision) throw new Error('Settings changed after editing began. Discard and retry.'); const next = draft(); const ops = Object.keys(next).filter((key) => JSON.stringify(base?.[key]) !== JSON.stringify(next[key])).map((key) => ({ op: 'set', path: [key], value: next[key] })); const secret = webhookUrl.trim(); validateWebhookDraft(secret); if (secret) ops.push({ op: 'set', path: ['webhookUrl'], value: secret }); if (ops.length) await scope.mutate(ops, revision); const accepted = verifyWrite(revision, ops); load(accepted.value, accepted.revision); setDirty(false); setSuccess(secret ? 'Settings saved. The webhook is stored securely.' : 'Settings saved.') } catch (error) { console.warn('[Discord Notify] settings save failed:', error); setFailed(error?.message || 'Could not save settings.') } finally { setSaving(false) } }
    const test = async () => { setTesting(true); setFailed(''); setSuccess(''); try { if (dirty) throw new Error('Save or discard your changes before sending a test.'); const before = scope.getSnapshot(); const nonce = Number.isSafeInteger(before.value?.testNonce) ? before.value.testNonce + 1 : 1; const ops = [{ op: 'set', path: ['testNonce'], value: nonce }]; await scope.mutate(ops, before.revision); const accepted = scope.getSnapshot(); if (accepted.revision === before.revision || accepted.value?.testNonce !== nonce) throw new Error('The Host rejected the test. Configure and save a Discord webhook URL first.'); load(accepted.value, accepted.revision); setSuccess('Test notification queued. Check Discord.') } catch (error) { setFailed(error?.message || 'Could not send test notification.') } finally { setTesting(false) } }
    ...
    return e('li', { style: card },
      e('button', { type: 'button', style: header, 'aria-expanded': open, 'aria-label': `${open ? 'Collapse' : 'Expand'}: Discord notifications`, onClick: () => setOpen(!open) }, e('span', { style: headText }, e('span', { style: titleStyle }, 'Discord notifications'), e('span', { style: desc }, 'Send one-way webhook alerts for turns, tools, and matching bash commands.')), dirty ? e('span', { style: badge }, 'Unsaved') : null, e(Chevron, { open })),
      open ? e('div', { style: body },
        e(Field, { id: 'discord-notify-webhook', label: 'Discord webhook URL', hint: 'Write-only secret. Leave blank to keep the current value, or paste a URL to set or replace it.' }, e('input', { id: 'discord-notify-webhook', type: 'password', autoComplete: 'off', placeholder: 'https://discord.com/api/webhooks/…', style: input, value: webhookUrl, disabled: saving, onChange: (event) => touch(setWebhookUrl, event.target.value) })),
        e(Field, { id: 'discord-notify-username', label: 'Webhook display name', hint: 'The sender name shown in Discord.' }, e('input', { id: 'discord-notify-username', style: input, maxLength: 80, value: username, disabled: saving, onChange: (event) => touch(setUsername, event.target.value) })),
