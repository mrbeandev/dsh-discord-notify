window.__ModuleLoader__.load({ id: 'dsh-discord-notify', factory: (require) => {
  ...
  function DiscordNotifyCard({ useScope, scope }) {
    ...
    const verifyWrite = (beforeRevision, ops) => { const accepted = scope.getSnapshot(); const visible = ops.filter((op) => op.path[0] !== 'webhookUrl'); if ((ops.length && accepted.revision === beforeRevision) || visible.some((op) => JSON.stringify(accepted.value?.[op.path[0]]) !== JSON.stringify(op.value))) throw new Error('The Host rejected the change. Check the webhook, regex, selected tools, and template variables.'); return accepted }

    const save = async () => { setSaving(true); setFailed(''); setSuccess(''); try { const before = scope.getSnapshot(); if (before.revision !== revision) throw new Error('Settings changed after editing began. Discard and retry.'); const next = draft(); const ops = Object.keys(next).filter((key) => JSON.stringify(base?.[key]) !== JSON.stringify(next[key])).map((key) => ({ op: 'set', path: [key], value: next[key] })); if (webhookUrl.trim()) ops.push({ op: 'set', path: ['webhookUrl'], value: webhookUrl.trim() }); if (ops.length) await scope.mutate(ops, revision); const accepted = verifyWrite(revision, ops); load(accepted.value, accepted.revision); setDirty(false); setSuccess('Settings saved.') } catch (error) { console.warn('[Discord Notify] settings save failed:', error); setFailed(error?.message || 'Could not save settings.') } finally { setSaving(false) } }
